package org.aksw.limes.core.measures.mapper.topology.contentsimilarity.evaluation.giant.datamodel;
...
public class RelatedGeometries {
    ...
    public boolean verifyRelation(Geometry sourceGeom, Geometry targetGeom, String relation) {
        try {
            final int dimension1 = sourceGeom.getDimension();
            final int dimension2 = targetGeom.getDimension();
            final IntersectionMatrix im = sourceGeom.relate(targetGeom);
            ...
            if (relation.equals(RADON.CONTAINS) && im.isContains()) {
                related = true;
            }
            if (relation.equals(RADON.COVEREDBY) && im.isCoveredBy()) {
                related = true;
            }
            if (relation.equals(RADON.COVERS) && im.isCovers()) {
                related = true;
            }
            if (relation.equals(RADON.CROSSES) && im.isCrosses(dimension1, dimension2)) {
                related = true;
            }
            if (relation.equals(RADON.EQUALS) && im.isEquals(dimension1, dimension2)) {
                related = true;
            }
            if (relation.equals(RADON.INTERSECTS) && im.isIntersects()) {
                related = true;
            }
            if (relation.equals(RADON.OVERLAPS) && im.isOverlaps(dimension1, dimension2)) {
                related = true;
            }
            if (relation.equals(RADON.TOUCHES) && im.isTouches(dimension1, dimension2)) {
                related = true;
            }
            if (relation.equals(RADON.WITHIN) && im.isWithin()) {
                related = true;
            }
